...
static void  TaskStart(void *pdata)
{
	...
	if (select == 1) {
		...
		for (i = 0; i < 4; i++)
		{
			mbox_to_random[i] = OSMboxCreate((void *)0); // mailbox 생성, random task로 보내는 mailbox
			mbox_to_decision[i] = OSMboxCreate((void *)0); // decision task로 보내는 mailbox
		}
	}
	else if (select == 2) {
		// 크기가 4인 queue OSQCreate사용해서 생성 (your code)
		queue_to_random = OSQCreate(&decision_queue[0], 4);
		queue_to_decision = OSQCreate(&random_queue[0], 4);
	}
	...
	if (*(char*)pdata == '4') {//decision task일 경우
		for (;;) {
			for (i = 0; i < N_TASK - 1; i++) {
				if (select == 1) {
					...
					// OSMboxPend로 mailbox 기다림
					get_number[i] = *(int*)OSMboxPend(mbox_to_decision[i], 0, &err); // random number 기다림
					// int 말고 *(int*)로 형변환 해야함
				}
				else if (select == 2) {
					// randomnumber가 넘어올 때 까지 기다림. get_number배열에 값 저장 (your code)
					// OSQPend로 message queue 기다림
			...
			for (i = 0; i < N_TASK - 1; i++) {
				...
				if (select == 1) {
					// (your code) mailbox에 'W'나 'L' post 하는 코드
					...
					// (your code) message queue에 'W'나 'L' post 하는 코드
		...
		for (;;) {
			...
			if (select == 1) {
				...
				// random number를 mailbox에 post하고 decision task로부터 'W'나 'L'을 기다리는 코드
				...
				// random task에서 decision task로 post 후 pend로 기다림 (your code)
				// random number를 message queue에 post하고 decision task로부터 'W'나 'L'을 기다리는 코드
